...
pub unsafe fn codegen(input: Vec<ParseItem::Statement>) {
...
unsafe fn codegen_expr(
    ...
    match expr {
        ...
        Statement::Expression(Expression::Infix(Infix::Plus, lhs, rhs)) => {
            let lhs = codegen_expr(context, builder, func, names, Statement::Expression(*lhs));
            let rhs = codegen_expr(context, builder, func, names, Statement::Expression(*rhs));
        ...
        Statement::Let(name, expr) => {
            let new_value =
                codegen_expr(context, builder, func, names, Statement::Expression(expr));
        ...
        Statement::Expression(Expression::Identifier(name)) => {
            println!(" yoyoyo {:?}", names);
            let pointer = names.get(&name).unwrap();
            let name = CString::new(name).unwrap();
            llvm::core::LLVMBuildLoad(builder, *pointer, name.as_ptr())
        }
        _ => llvm::core::LLVMConstInt(llvm::core::LLVMInt64TypeInContext(context), 0, 0), // Expr::Mul(lhs, rhs) => {
